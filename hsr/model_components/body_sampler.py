...
class PointOnBones:
    ...
    def get_points(self, joints, num_per_bone=5):
        ...
        for bone_id in self.bone_ids:
            ...
            scalars = (
                scalars + torch.randn((num_batch, num_per_bone), device=joints.device) * 0.001
            ).clamp_(0, 1)

            samples.append(
                joints[:, bone_id[0]].unsqueeze(1).expand(-1, scalars.shape[-1], -1)
                + torch.einsum(
                    "bn,bi->bni", scalars, bone_dir
                )  # b: num_batch, n: num_per_bone, i: 3-dim
            )

        samples = torch.cat(samples, dim=1)

        probs = torch.ones((num_batch, samples.shape[1]), device=joints.device) * 0.01
...
class PointInSpace:
    ...
    def get_points(self, pc_input, local_sigma=None, global_ratio=0.125):
